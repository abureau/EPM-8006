...
fram1 = read.csv("/Users/alexandrebureau/Documents/MonTravail/cours/Concepts avancés en modélisation statistique/Données/fram1.csv");
```
...
```{r}
library(MASS)
res.std = studres(reglin); # residus standardises
plot(y = res.std, x = fram1$AGE, pch = 16);
lines(lowess(y = res.std, x = fram1$AGE), col = "brown", lwd = 3);
...
```
On peut aussi utiliser la fonction *geeglm* pour obtenir l'estimation robuste de la variance en précisant que chaque individu forme sa propre grappe.
```{r}
library(geepack)
reglinEE = geeglm(SYSBP~SEX+AGE+BMI,data=fram1,id=RANDID)
summary(reglinEE)
```
...
```{r}
dfb = dfbetas(reglin);
dfb[,"BMI"][abs(dfb[,"BMI"]) > 0.2]
fram1$RANDID[abs(dfb[,"BMI"]) > 0.2]
```
