...
{
  bd<-data[indices,]
  model = glm(DIABETES~SEX*CURSMOKE+AGE+BMI+SYSBP+DIABP,data=bd,family = poisson(link=log))
  # Les prochaines lignes sont tirées de la fonction epi.interaction du module epiR
        coef=c(2,3,8)
        ...
        exp(theta1 + theta2 + theta3) - exp(theta1) - 
            exp(theta2) + 1
}
```
...
{
  model = glm(DIABETES~SEX*CURSMOKE+AGE+BMI+SYSBP+DIABP,data=data,weights=weights,family = poisson(link=log))
  # Les prochaines lignes sont tirées de la fonction epi.interaction du module epiR
        coef=c(2,3,8)
        theta1 <- as.numeric(model$coefficients[coef[1]])
        theta2 <- as.numeric(model$coefficients[coef[2]])
...
RERIsexe_tabac.IC
```
Par contre, le calcul fonctionne sur l'objet obtenu avec la fonction qui prend des poids.
```{r}
