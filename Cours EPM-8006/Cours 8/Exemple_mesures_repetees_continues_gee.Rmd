...
On peut obtenir des intervalles de confiance avec la fonction *confint.default* (Attention! *confint* tout court produit une erreur). Malheureusement, ces fonctions ne s'appliquent pas aux objets produits par *geeM*. La fonction *glht* que nous avons utilisée précédemment pour estimer des combinaisons linéaires avec des modèles linéaires ne s'applique ni aux objets de *geeglm*, ni aux objets de *geeM*.

Nous allons appliquer *confint.default* aux objets de *geeglm* pour fin d'illustration, mais les résultats peuvent être invalides. 
```{r}
confint.default(modGEE.ar1);
```

## Estimation d'un modèle sous indépendance
```{r}
library(geeM);
modGEEm.ind = geem(Counts ~ TX + Visit + TX*Visit + Counts0, data = seizure3, family = gaussian, id = ID, corstr = "ind");
summary(modGEEm.ind);
```

# Estimation de comparaisons prédéterminées
On spécifie successivement la réponse moyenne sous traitement à la visite 1, la réponse moyenne sous placebo à la visite 1 et la différence traitement - placebo à la visite 1. On peut estimer des combinaisons linéaires à partir des objets de *geeglm* avec la fonction *esticon*. 
```{r}
...
```
Pour la moyenne dans chaque groupe du style lsmeans de SAS:
```{r}
emmeans(modGEE.ar1,"TX",by="Visit")
```
