...
```{r}
ctab = coef(summary(hanche.approxmod))
exp(cbind(ctab[2:3,"Estimate"],ctab[2:3,"Estimate"]-qnorm(0.975)*ctab[2:3,"Std. Error"],ctab[2:3,"Estimate"]+qnorm(0.975)*ctab[2:3,"Std. Error"]))
```
...
```{r}
hanche.bayesmod2b = MCMClogit(hanche~cons_reg+cons_var+OSQ130+OSQ170+OSQ200+RIAGENDR+factor(RIDRETH1)+RIDAGEYR+ALQ101+ALQ130+BMXBMI+MCQ160C+MCQ160L+WHD020+WHD110,data=osteo,burnin = 1000, mcmc = 1000000, thin = 100, b0=moy.priori,B0 = 1/var.priori, seed=331133)
...
# Intervalles de crédibilité
Approximation normale (pas conseillée)
```{r}
stats.tab = summary(hanche.bayesmod2b)$statistics
exp(cbind(stats.tab[2:3,"Mean"],stats.tab[2:3,"Mean"]-qnorm(0.975)*stats.tab[2:3,"SD"],stats.tab[2:3,"Mean"]+qnorm(0.975)*stats.tab[2:3,"SD"]))
```
Quantiles de la distribution a posteriori (préférable)
```{r}
qtab = summary(hanche.bayesmod2b)$quantiles
exp(cbind(stats.tab[2:3,"Mean"],qtab[2:3,"2.5%"],qtab[2:3,"97.5%"]))
```