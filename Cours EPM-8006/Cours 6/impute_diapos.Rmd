...
aggr(osteo2)
```
Proportion d'observations complètes et avec données manquantes
```{r}
prop.table(table(complete.cases(osteo2)))
```
...
On ne peut pas utiliser *with()* avec *geeglm()* ou *geem*, il faut donc programmer soi-même une boucle sur les imputations.
```{r}
require(geeM)
estimates = std.err = numeric(imp$m);
for(i in 1:imp$m){
  ds = complete(imp, i);
  # Il reste des valeurs manquantes pour 3 observations
  ds = ds[complete.cases(ds),]
  ds$fracture = as.numeric(as.character(ds$fracture));   
  fit = geem(fracture ~ cons + OSQ130 + OSQ170 +
                          OSQ200 + BMXBMI +
                          WHD020 + WHD110 + ALQ101 + ALQ130 + 
                          RIAGENDR + RIDAGEYR + RIDRETH1,
               family = poisson(link = "log"), id = SEQN, data = ds,scale.fix=T);
  estimates[i] = coef(fit)[2];
  std.err[i] = summary(fit)$se.robust[2];
}
...
final.results[1,] = c(res$qbar, sqrt(res$t));
